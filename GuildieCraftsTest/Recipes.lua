...
local function EnsureDB()
    ...
    GuildieCraftsTestDB.recipes.jcReports = GuildieCraftsTestDB.recipes.jcReports or {}
end
...
local function GetProfessionLabel(professionId)
    return GuildieCraftsTest_GetProfessionLabel(professionId or "jewelcrafting")
end

function GuildieCraftsTest_ShouldShareRecipes()
    return GuildieCraftsTest_ShouldShareStock()
end

function GuildieCraftsTest_IsJewelcraftingOpen()
    return GuildieCraftsTest_IsTradeSkillOpen("jewelcrafting")
end

function GuildieCraftsTest_IsTradeSkillOpen(professionId)
...
function GuildieCraftsTest_HandleRecipesRefreshClick()
    local professionId = GetActiveRecipeProfession()
    if GuildieCraftsTest_IsTradeSkillOpen(professionId) then
        GuildieCraftsTest_RefreshRecipesFromMacro(false)
        return
    end
    ...
    GuildieCraftsTest_RefreshRecipesFromMacro(true)
end

function GuildieCraftsTest_InitRecipesRefreshButton(btn)
    ...
    btn:SetAttribute("macrotext", "/run GuildieCraftsTest_HandleRecipesRefreshClick()")
end

function GuildieCraftsTest_RefreshRecipesSyncOnly()
    if GuildieCraftsTest.Sync then
        GuildieCraftsTest.Sync:RequestSync()
    end
    if GuildieCraftsTest.UI then
        GuildieCraftsTest_RefreshUI()
    end
    print("|cff00ccffGuildieCraftsTest|r Recipe list refreshed.")
end
...
local function MatchGemByName(name)
    ...
    local gem = GuildieCraftsTest_GemByName[name]
    if gem then
        return gem
    end
    if strlower then
        for gemName, entry in pairs(GuildieCraftsTest_GemByName or {}) do
...
local function MatchCraftByName(name)
    ...
    local craft = GuildieCraftsTest_GetCraftByName(name)
    if craft then
        return craft
    end
    if strlower then
        for craftName, entry in pairs(GuildieCraftsTest_CraftByName or {}) do
...
function GuildieCraftsTest_ScanOpenTradeSkill()
    local known = {}
    ...
    for i = 1, num do
        ...
        if name and skillType ~= "header" then
            ...
            if itemId and GuildieCraftsTest_IsTrackedRecipeItemId(itemId) then
                known[itemId] = true
            end
        end
    end
...
local function StoreLocalRecipeReport(itemIds)
    EnsureDB()
    GuildieCraftsTestDB.recipes.jcReports[UnitName("player")] = {
        itemIds = itemIds,
        updatedAt = time(),
    }
end
...
local function ApplyRecipeScanResult(known, numSkills, source)
    ...
    GuildieCraftsTest.Sync:BroadcastRecipes(known)
    if GuildieCraftsTest.UI then
        GuildieCraftsTest_RefreshUI()
    end
    ...
    if scanPending then
        if numSkills <= 0 then
            print("|cffff0000GuildieCraftsTest|r Could not read recipes. Open your "
                .. professionLabel .. " window and try again.")
        elseif knownCount == 0 then
            print("|cff00ccffGuildieCraftsTest|r Scanned " .. professionLabel
                .. " (" .. numSkills .. " entries). No tracked recipes found yet.")
        else
            print("|cff00ccffGuildieCraftsTest|r Scanned and shared " .. knownCount
                .. " recipes with the workshop.")
        end
    elseif source == "login" and knownCount > 0 then
        print("|cff00ccffGuildieCraftsTest|r Shared " .. knownCount .. " recipes with the workshop.")
    end
...
local function RunRecipeScan(source)
    if not GuildieCraftsTest_ShouldShareRecipes() then
        scanPending = false
        FinishAutoScanClose()
        return
    end

    local known, numSkills = GuildieCraftsTest_ScanOpenTradeSkill()
    if numSkills > 0 then
...
function GuildieCraftsTest_ScanAndShareRecipes()
    EnsureDB()
    if not GuildieCraftsTest_ShouldShareRecipes() then
        local crafters = string.lower(GuildieCraftsTest_GetCrafterLabel(GetActiveRecipeProfession()))
    ...
    print("|cff00ccffGuildieCraftsTest|r Open "
        .. GetProfessionLabel(GetActiveRecipeProfession())
        .. ", then click Refresh on the Recipes tab.")
    return false, "closed"
end

function GuildieCraftsTest_ShareWorkshopRecipes()
    EnsureDB()
    if not GuildieCraftsTest_ShouldShareRecipes() then
        return
    end
    local player = UnitName("player")
    local report = GuildieCraftsTestDB.recipes.jcReports[player]
    if report and report.itemIds then
        GuildieCraftsTest.Sync:BroadcastRecipes(report.itemIds)
    end
end

function GuildieCraftsTest_AutoScanRecipes(silent)
    if not GuildieCraftsTest_ShouldShareRecipes() then
        return
    end

    local professionId = GetActiveRecipeProfession()
    if not GuildieCraftsTest_IsTradeSkillOpen(professionId) then
        if not silent then
            print("|cff00ccffGuildieCraftsTest|r Open "
...
function GuildieCraftsTest_RequestRecipeScan()
    return GuildieCraftsTest_ScanAndShareRecipes()
end

function GuildieCraftsTest_ApplyRecipeReport(player, itemIds)
    EnsureDB()
    GuildieCraftsTestDB.recipes.jcReports[player] = {
        itemIds = itemIds,
        updatedAt = time(),
    }
    if GuildieCraftsTest.UI then
...
function GuildieCraftsTest_GetRecipeCoverage(room, tier)
    EnsureDB()
    local coverage = {}
    if not room then
        return coverage
    ...
    local profession = GuildieCraftsTest_GetRoomProfession(room)
    if profession ~= "jewelcrafting" then
        for _, craft in ipairs(GuildieCraftsTest_GetCraftCatalog(profession)) do
            local jcs = {}
            for _, crafterName in ipairs(GuildieCraftsTest_GetWorkshopStockContributors(room)) do
    ...
    for _, gem in ipairs(GuildieCraftsTest_GetRecipeGems(tier or "epic")) do
        local jcs = {}
        for _, jcName in ipairs(GuildieCraftsTest_GetWorkshopStockContributors(room)) do
            local report = GuildieCraftsTestDB.recipes.jcReports[jcName]
...
function GuildieCraftsTest_WorkshopHasRecipeForGem(gemName)
    if not gemName or gemName == "None" then
        return true
    end

    local gem = GuildieCraftsTest_GemByName[gemName]
...
function GuildieCraftsTest_GetRecipeIndicatorText(gemName)
    if not gemName or gemName == "None" then
        return ""
    end

    local gem = GuildieCraftsTest_GemByName and GuildieCraftsTest_GemByName[gemName]
    if not gem or gem.raw or not gem.itemId then
        return ""
    end

    if GuildieCraftsTest_WorkshopHasRecipeForGem(gemName) then
...
function GuildieCraftsTest_FormatGemDropdownLabel(gemName)
    if not gemName or gemName == "None" then
        return nil
    end

    local indicator = GuildieCraftsTest_GetRecipeIndicatorText(gemName)
    if indicator == "" then
        return gemName
    end
...
function GuildieCraftsTest_GetCraftRecipeIndicatorText(craftName)
    ...
    if GuildieCraftsTest_WorkshopHasRecipeForCraft(craftName) then
        return RECIPE_KNOWN_ICON .. " "
    end
...
function GuildieCraftsTest_FormatCraftDropdownLabel(craft)
    if not craft then
        return ""
    end

    local base = GuildieCraftsTest_FormatCraftLabel(craft)
    local indicator = GuildieCraftsTest_GetCraftRecipeIndicatorText(craft.name)
    if indicator == "" then
        return base
    end
...
local function QueuePassiveRecipeScan()
    if not GuildieCraftsTest_ShouldShareRecipes() or not GuildieCraftsTest_HasJoinedWorkshop() then
        return
    end
    ...
    local function run()
        ...
        if not GuildieCraftsTest_ShouldShareRecipes() or not GuildieCraftsTest_IsTradeSkillOpen(GetActiveRecipeProfession()) then
            return
        end
        local known, numSkills = GuildieCraftsTest_ScanOpenTradeSkill()
        if numSkills <= 0 then
...
function GuildieCraftsTest_RefreshRecipesFromMacro(delayScan)
    if not GuildieCraftsTest_ShouldShareRecipes() then
        GuildieCraftsTest_RefreshRecipesSyncOnly()
        return
    end

    if delayScan then
        scanPending = true
        print("|cff00ccffGuildieCraftsTest|r Opening "
            .. GetProfessionLabel(GetActiveRecipeProfession()) .. "…")
    ...
    if numSkills > 0 then
        ...
        print("|cffff0000GuildieCraftsTest|r Open your "
            .. GetProfessionLabel(GetActiveRecipeProfession())
...
function GuildieCraftsTest_InitRecipeEvents()
