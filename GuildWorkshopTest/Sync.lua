...
function Sync:BroadcastAllOrderQueues(requester)
    for _, room in pairs(GuildWorkshopTestDB.rooms or {}) do
        if room.id and room.orderQueue and GuildWorkshopTest_IsRoomMember(room, requester) then
...
function Sync:BroadcastRoom(room)
    ...
    local msg = table.concat({
        ...
        EncodeField(room.expansion or GuildWorkshopTest_GetDefaultExpansion()),
        EncodeField(room.phase or GuildWorkshopTest_GetDefaultPhase()),
...
function Sync:OnAddonMessage(prefix, message, channel, sender)
    ...
    if msgType == MSG_ADD then
        ...
        local room = roomId and GuildWorkshopTest_GetRoom(roomId)
        if not room or not GuildWorkshopTest_IsSameGuild(room) then
            return
        end
        if not GuildWorkshopTest_IsRoomMember(room, UnitName("player")) then
            return
        end
        local order = {
            id = parts[2],
        ...
        local existing = GuildWorkshopTestDB.orders[order.id]
        if existing then
    ...
    elseif msgType == MSG_REQUEST then
        ...
        for _, order in pairs(GuildWorkshopTestDB.orders or {}) do
            if order.roomId then
                ...
                if room and GuildWorkshopTest_IsSameGuild(room) and GuildWorkshopTest_IsRoomMember(room, requester) then
                    self:BroadcastOrder(order)
                end
            end
        end
        ...
        if GuildWorkshopTest_ShouldShareRecipes() then
            GuildWorkshopTest_ShareWorkshopRecipes()
        end
        self:BroadcastAllOrderQueues(requester)
    elseif msgType == MSG_ROOM then
        local guild = DecodeField(parts[8])
        if guild and guild ~= GuildWorkshopTest_GetGuildName() then
            return
        end
        ...
        local room = {
            ...
            guild = guild or GuildWorkshopTest_GetGuildName(),
            expansion = DecodeField(parts[11]) or GuildWorkshopTest_GetDefaultExpansion(),
            phase = DecodeField(parts[12]) or GuildWorkshopTest_GetDefaultPhase(),
            profession = DecodeField(parts[13]) or "jewelcrafting",
        }
        room.members[room.leader] = true
        GuildWorkshopTest_ApplyRoom(room)
    elseif msgType == MSG_JOIN then
        local room = GuildWorkshopTestDB.rooms[parts[2]]
        if not room then
        ...
        GuildWorkshopTest_ApplyRoom(room)
        if GuildWorkshopTest_IsRoomMember(room, UnitName("player")) then
            self:BroadcastRoom(room)
            if GuildWorkshopTest_CanManageWorkshop(room, UnitName("player")) then
    ...
    elseif msgType == MSG_COLLAB then
        ...
        if room then
            ...
            if parts[3] == "add" then
                ...
                if parts[4] == UnitName("player") and GuildWorkshopTest_ShouldShareStock() then
    ...
    elseif msgType == MSG_QUEUE then
        local room = GuildWorkshopTestDB.rooms[parts[2]]
        if room and GuildWorkshopTest_IsRoomMember(room, UnitName("player")) then
    ...
    elseif msgType == MSG_STOCK then
        ...
        if not room or not GuildWorkshopTest_HasJoinedWorkshop() then
            return
        end
        if not GuildWorkshopTest_AcceptsWorkshopStockReport(room, parts[2]) then
            return
        end
        GuildWorkshopTest_ApplyStockReport(parts[2], DecodeStock(parts[4]), parts[3])
    elseif msgType == MSG_RECIPES then
        ...
        GuildWorkshopTest_ApplyRecipeReport(parts[2], DecodeRecipeIds(parts[3]))
    end
end
