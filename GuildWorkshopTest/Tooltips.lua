...
function GuildWorkshopTest_IsLoggingOut()
    return GuildWorkshopTest._loggingOut == true
end

function GuildWorkshopTest_PrepareForLogout()
    GuildWorkshopTest._loggingOut = true

    if UISpecialFrames then
        for i = #UISpecialFrames, 1, -1 do
            local entry = UISpecialFrames[i]
            if entry == "GuildWorkshopTestFrame" or entry == "GuildWorkshopTestOrderDialog" then
                tremove(UISpecialFrames, i)
            end
        end
    end

    local ui = GuildWorkshopTest.UI
...
function GuildWorkshopTest_ShowItemTooltip(owner, itemId, anchor)
...
function GuildWorkshopTest_ShowLinkTooltip(owner, link, anchor)
...
function GuildWorkshopTest_HideTooltip()
    GameTooltip:Hide()
end
...
function GuildWorkshopTest_InsertChatItemLink(itemIdOrLink)
...
function GuildWorkshopTest_HandleItemClick(itemIdOrLink, button)
    if button ~= "LeftButton" and button ~= "LeftButtonUp" then
        return false
    end
    ...
    if chatLink then
        return GuildWorkshopTest_InsertChatItemLink(itemIdOrLink)
    end

    return false
end

function GuildWorkshopTest_RegisterEscapeFrame(frame)
    if not frame or not UISpecialFrames then
        return
    end
    if GuildWorkshopTest.Debug and GuildWorkshopTest.Debug.ShouldSkipUISpecialFrames
        and GuildWorkshopTest.Debug:ShouldSkipUISpecialFrames() then
        return
    end
...
function GuildWorkshopTest_GetGemItemId(gemName)
...
function GuildWorkshopTest_ExtractItemLink(text)
...
function GuildWorkshopTest_ExtractItemId(text)
    local link = GuildWorkshopTest_ExtractItemLink(text)
    if not link then
        return nil
    end
...
function GuildWorkshopTest_GetItemDisplayText(text)
    local link = GuildWorkshopTest_ExtractItemLink(text)
    if link then
...
function GuildWorkshopTest_TooltipsInit()
    GuildWorkshopTest.itemLinkTarget = nil
end

function GuildWorkshopTest_AttachItemTooltip(frame, getLinkFn)
    frame:EnableMouse(true)
    frame:SetScript("OnEnter", function(self)
        local link = getLinkFn(self)
        if link then
            if type(link) == "number" then
                GuildWorkshopTest_ShowItemTooltip(self, link)
            else
                GuildWorkshopTest_ShowLinkTooltip(self, link)
            end
        end
    end)
    frame:SetScript("OnLeave", GuildWorkshopTest_HideTooltip)
    frame:SetScript("OnMouseUp", function(self, button)
        GuildWorkshopTest_HandleItemClick(getLinkFn(self), button)
    end)
end

function GuildWorkshopTest_ClearDropdownItemTooltips(level)
    ...
    for i = 1, maxButtons do
        ...
        if button then
            button.GuildWorkshopItemId = nil
        end
    end
end

function GuildWorkshopTest_AttachDropdownItemButton(itemId, buttonIndex)
    ...
    button.GuildWorkshopItemId = itemId
    button:SetScript("OnEnter", function(self)
        if UIDropDownMenuButton_OnEnter then
            UIDropDownMenuButton_OnEnter(self)
        end
        local linkedItemId = self.GuildWorkshopItemId
        if linkedItemId
            and type(self.value) == "number"
            and self.value > 0
            and self.value == linkedItemId then
            GuildWorkshopTest_ShowItemTooltip(self, linkedItemId, "ANCHOR_RIGHT")
        end
    end)
    button:SetScript("OnLeave", function(self)
        GuildWorkshopTest_HideTooltip()
    ...
    button:SetScript("OnMouseUp", function(self, mouseButton)
        ...
        if linkedItemId
            ...
            and self.value > 0
            and self.value == linkedItemId then
            GuildWorkshopTest_HandleItemClick(linkedItemId, mouseButton)
        end
    end)
end

function GuildWorkshopTest_ApplyDropdownItemTooltips(indexToItemId)
    ...
    local function apply()
        GuildWorkshopTest_ClearDropdownItemTooltips(level)
        for index, itemId in pairs(indexToItemId) do
            GuildWorkshopTest_AttachDropdownItemButton(itemId, index)
        end
    end
...
function GuildWorkshopTest_AttachDropdownTooltips(dropdown, getItemIdFn)
    local button = _G[dropdown:GetName() .. "Button"]
    if button then
        GuildWorkshopTest_AttachItemTooltip(button, function()
            return getItemIdFn()
        end)
    end
end

function GuildWorkshopTest_HookDropdownMenuTooltips()
