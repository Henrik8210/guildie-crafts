...
function UI:CreateWorkshopPanel()
        ...
        for _, openRoom in ipairs(UI.joinRoomOptions or {}) do
            ...
            info.func = function()
                ...
                if err then
                    ...
                    print("|cff00ccffGuildie Crafts|r Selected " .. openRoom.name .. ".")
    ...
    panel.changeWorkshopBtn:SetScript("OnClick", function()
        ...
        if err then
            ...
            print("|cff00ccffGuildie Crafts|r Workshop deselected. You remain a member — select another below.")
...
function UI:PopulateCraftDropdown(dropdown, frameRef)
    ...
    for _, craft in ipairs(GuildieCrafts_GetCraftsByCategory(professionId, category)) do
        local info = UIDropDownMenu_CreateInfo()
        info.text = GuildieCrafts_FormatCraftLabel(craft)
...
function UI:CreateOrderRow(order, yOffset)
    ...
    if order.status == "in_progress" then
        title:SetText(string.format(
            ...
            GuildieCrafts_GetRoleLabel(order.role)
        ))
    else
        title:SetText(string.format(
            "%s  %s  [%s]",
