...
GuildieCrafts_CutGemsCache = nil
GuildieCrafts_RareCutGemsCache = nil

GuildieCrafts_GemByName = {}
GuildieCrafts_GemByItemId = {}
for _, gem in ipairs(GuildieCrafts_Gems) do
    GuildieCrafts_GemByName[gem.name] = gem
    if gem.itemId then
        GuildieCrafts_GemByItemId[gem.itemId] = gem
    end
end

function GuildieCrafts_GetCutGems()
    if GuildieCrafts_CutGemsCache then
        return GuildieCrafts_CutGemsCache
    end
    local list = {}
    for _, gem in ipairs(GuildieCrafts_Gems) do
    ...
    GuildieCrafts_CutGemsCache = list
    return list
end

function GuildieCrafts_GetEpicCutGems()
    return GuildieCrafts_GetCutGems()
end

function GuildieCrafts_GetRareCutGems()
    if GuildieCrafts_RareCutGemsCache then
        return GuildieCrafts_RareCutGemsCache
    end
    local list = {}
    for _, gem in ipairs(GuildieCrafts_Gems) do
    ...
    GuildieCrafts_RareCutGemsCache = list
    return list
end

function GuildieCrafts_GetRawGems()
    local list = {}
    for _, gem in ipairs(GuildieCrafts_Gems) do
        if gem.raw then
            table.insert(list, gem)
...
function GuildieCrafts_GetGemColor(name)
    local gem = GuildieCrafts_GemByName[name]
    return gem and gem.color or "Unknown"
end

function GuildieCrafts_GetRecipeGems(tier)
    if tier == "rare" then
        return GuildieCrafts_GetRareCutGems()
    end
    return GuildieCrafts_GetEpicCutGems()
end

function GuildieCrafts_IsTrackedRecipeItemId(itemId)
    local gem = GuildieCrafts_GemByItemId[itemId]
    if gem and not gem.raw then
        return true
    end
    if GuildieCrafts_IsTrackedHoDCraftItemId then
        return GuildieCrafts_IsTrackedHoDCraftItemId(itemId)
    end
    return false
end

function GuildieCrafts_FormatGemLabel(gem)
    return gem.name
end
