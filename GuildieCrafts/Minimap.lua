...
local MinimapBtn = {}
GuildieCrafts.Minimap = MinimapBtn
...
local function GetAngle()
    ...
    return GuildieCraftsDB.settings.minimapAngle or 220
end

local function SetAngle(angle)
    GuildieCraftsDB.settings = GuildieCraftsDB.settings or {}
    GuildieCraftsDB.settings.minimapAngle = angle
end
...
function MinimapBtn:Init()
    ...
    local button = CreateFrame("Button", "GuildieCraftsMinimapButton", Minimap)
    ...
    icon:SetTexture(GuildieCrafts_GetDefaultWorkshopIcon())
    ...
    button:SetScript("OnClick", function(_, mouseButton)
        ...
        if not GuildieCrafts_EnsureUI() then
            return
        end
        if mouseButton == "RightButton" then
            if GuildieCrafts.UI then
                GuildieCrafts.UI:ShowTab("workshop")
                GuildieCrafts.UI:Show()
            end
            return
        end
    ...
    button:SetScript("OnEnter", function(self)
        ...
        local pending = GuildieCrafts_GetPendingNotifications and GuildieCrafts_GetPendingNotifications() or {}
        if #pending > 0 then
            GameTooltip:AddLine(" ")
...
function MinimapBtn:StopPulse()
    ...
    if GuildieCrafts_ClearPendingNotifications then
        GuildieCrafts_ClearPendingNotifications()
    end
end
