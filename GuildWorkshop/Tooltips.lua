...
GuildWorkshop._loggingOut = false

function GuildWorkshop_IsLoggingOut()
    return GuildWorkshop._loggingOut == true
end

function GuildWorkshop_PrepareForLogout()
    GuildWorkshop._loggingOut = true

    if UISpecialFrames then
        for i = #UISpecialFrames, 1, -1 do
            local entry = UISpecialFrames[i]
            if entry == "GuildWorkshopFrame" or entry == "GuildWorkshopOrderDialog" then
                tremove(UISpecialFrames, i)
            end
        end
    end

    local ui = GuildWorkshop.UI
...
function GuildWorkshop_ShowItemTooltip(owner, itemId, anchor)
...
function GuildWorkshop_ShowLinkTooltip(owner, link, anchor)
...
function GuildWorkshop_HideTooltip()
    GameTooltip:Hide()
end
...
function GuildWorkshop_InsertChatItemLink(itemIdOrLink)
...
function GuildWorkshop_HandleItemClick(itemIdOrLink, button)
    if button ~= "LeftButton" and button ~= "LeftButtonUp" then
        return false
    end
    ...
    if chatLink then
        return GuildWorkshop_InsertChatItemLink(itemIdOrLink)
    end

    return false
end

function GuildWorkshop_RegisterEscapeFrame(frame)
    if not frame or not UISpecialFrames then
        return
    end
    if GuildWorkshop.Debug and GuildWorkshop.Debug.ShouldSkipUISpecialFrames
        and GuildWorkshop.Debug:ShouldSkipUISpecialFrames() then
        return
    end
...
function GuildWorkshop_GetGemItemId(gemName)
...
function GuildWorkshop_ExtractItemLink(text)
...
function GuildWorkshop_ExtractItemId(text)
    local link = GuildWorkshop_ExtractItemLink(text)
    if not link then
        return nil
    end
...
function GuildWorkshop_GetItemDisplayText(text)
    local link = GuildWorkshop_ExtractItemLink(text)
    if link then
...
function GuildWorkshop_TooltipsInit()
    GuildWorkshop.itemLinkTarget = nil
end

function GuildWorkshop_AttachItemTooltip(frame, getLinkFn)
    frame:EnableMouse(true)
    frame:SetScript("OnEnter", function(self)
        local link = getLinkFn(self)
        if link then
            if type(link) == "number" then
                GuildWorkshop_ShowItemTooltip(self, link)
            else
                GuildWorkshop_ShowLinkTooltip(self, link)
            end
        end
    end)
    frame:SetScript("OnLeave", GuildWorkshop_HideTooltip)
    frame:SetScript("OnMouseUp", function(self, button)
        GuildWorkshop_HandleItemClick(getLinkFn(self), button)
    end)
end

function GuildWorkshop_ClearDropdownItemTooltips(level)
...
function GuildWorkshop_AttachDropdownItemButton(itemId, buttonIndex)
    ...
    button:SetScript("OnEnter", function(self)
        ...
        if linkedItemId
            ...
            GuildWorkshop_ShowItemTooltip(self, linkedItemId, "ANCHOR_RIGHT")
        end
    end)
    button:SetScript("OnLeave", function(self)
        GuildWorkshop_HideTooltip()
    ...
    button:SetScript("OnMouseUp", function(self, mouseButton)
        ...
        if linkedItemId
            ...
            and self.value > 0
            and self.value == linkedItemId then
            GuildWorkshop_HandleItemClick(linkedItemId, mouseButton)
        end
    end)
end

function GuildWorkshop_ApplyDropdownItemTooltips(indexToItemId)
    ...
    local function apply()
        GuildWorkshop_ClearDropdownItemTooltips(level)
        for index, itemId in pairs(indexToItemId) do
            GuildWorkshop_AttachDropdownItemButton(itemId, index)
        end
    end
...
function GuildWorkshop_AttachDropdownTooltips(dropdown, getItemIdFn)
    local button = _G[dropdown:GetName() .. "Button"]
    if button then
        GuildWorkshop_AttachItemTooltip(button, function()
            return getItemIdFn()
        end)
    end
end

function GuildWorkshop_HookDropdownMenuTooltips()
