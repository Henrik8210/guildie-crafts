...
local MinimapBtn = {}
GuildWorkshop.Minimap = MinimapBtn
...
local function GetAngle()
    ...
    return GuildWorkshopDB.settings.minimapAngle or 220
end

local function SetAngle(angle)
    GuildWorkshopDB.settings = GuildWorkshopDB.settings or {}
    GuildWorkshopDB.settings.minimapAngle = angle
end
...
function MinimapBtn:Init()
    ...
    local button = CreateFrame("Button", "GuildWorkshopMinimapButton", Minimap)
    ...
    button:SetScript("OnClick", function(_, mouseButton)
        if not GuildWorkshop_EnsureUI() then
            return
        end
        if mouseButton == "RightButton" then
            if GuildWorkshop.UI then
                GuildWorkshop.UI:ShowTab("workshop")
                GuildWorkshop.UI:Show()
            end
            return
        end
    ...
    button:SetScript("OnEnter", function(self)
        ...
        GameTooltip:SetText("GuildWorkshop", 1, 1, 1)
