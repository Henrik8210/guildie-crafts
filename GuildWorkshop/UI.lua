...
local function RegisterCloseWorkshopPopup()
    if GuildWorkshop.closeWorkshopPopupRegistered then
        return
    end
    GuildWorkshop.closeWorkshopPopupRegistered = true
    StaticPopupDialogs = StaticPopupDialogs or {}
    StaticPopupDialogs["GuildWorkshop_CONFIRM_CLOSE_WORKSHOP"] = {
        ...
        OnAccept = function(self)
            ...
            local ok, err = GuildWorkshop_CloseWorkshop(data.roomId)
            if not ok then
                print("|cff00ccffGuild Workshop|r " .. (err or "Could not close workshop."))
            else
                print("|cff00ccffGuild Workshop|r Workshop closed.")
            end
...
local function ConfirmCloseWorkshop(room)
    ...
    local dialog = StaticPopup_Show("GuildWorkshop_CONFIRM_CLOSE_WORKSHOP", room.name)
    if dialog then
        dialog.data = {
            roomId = room.id,
            roomName = room.name,
...
local function ColorizeGem(name)
    local color = GEM_COLORS[GuildWorkshop_GetGemColor(name)] or GEM_COLORS.Unknown
    return string.format("|cff%s%s|r", color, name)
end
...
local function LayoutQueueScrollbarOnce(inset)
    local scrollBar = _G["GuildWorkshopScrollScrollBar"]
    if scrollBar then
...
local function CreateMainFrameHelper()
    local ok, frame = pcall(CreateFrame, "Frame", "GuildWorkshopFrame", UIParent, "PortraitFrameTemplate")
    if ok and frame then
        return frame
    end
    frame = CreateFrame("Frame", "GuildWorkshopFrame", UIParent)
    if frame.SetBackdrop then
        frame:SetBackdrop({
            bgFile = "Interface\\DialogFrame\\UI-DialogBox-Background",
...
local function HideOrderDialogSilently()
    if not GuildWorkshop.UI or not GuildWorkshop.UI.frame then
        return
    end
    local f = GuildWorkshop.UI.frame
...
local function GetAddonVersion()
    if GuildWorkshop_GetVersion then
        return GuildWorkshop_GetVersion()
    end
...
function UI:BuildDropdownCaches()
    ...
    for _, gem in ipairs(GuildWorkshop_GetCutGems()) do
...
function UI:RefreshJoinDropdownCache()
    self.joinRoomOptions = GuildWorkshop_GetOpenRooms()
end
...
function UI:CreateMainFrame()
    ...
    GuildWorkshop_RegisterEscapeFrame(f)
    ...
    f.creditsDev = CreateLabel(f, "Developed by " .. GuildWorkshop_ColorizePlayer("Nobunda", "SHAMAN"), "GameFontDisableSmall")
    f.creditsDev:SetPoint("BOTTOM", 0, 22)
    f.creditsDev:SetJustifyH("CENTER")

    f.creditsTest = CreateLabel(f, "Tested by " .. GuildWorkshop_ColorizePlayer("Justtwo", "WARLOCK"), "GameFontDisableSmall")
...
function UI:UpdateOrdersLayout(tabId)
    ...
    if tabId == "orders" then
        ...
        if f.placeOrderBtn then
            if room and GuildWorkshop_IsWorkshopFullySupported(room) then
...
function UI:UpdateTabAccess()
    local joined = GuildWorkshop_HasJoinedWorkshop()
    local f = self.frame
...
function UI:ShowTab(tabId, skipAccessCheck)
    ...
    if not skipAccessCheck and (tabId == "orders" or tabId == "completed" or tabId == "stock" or tabId == "recipes") and not GuildWorkshop_HasJoinedWorkshop() then
        tabId = "workshop"
    end
    ...
    if (tabId == "orders" or tabId == "completed" or tabId == "recipes") and GuildWorkshop_HasJoinedWorkshop() and IsInGuild() then
...
function UI:CreateWorkshopPanel()
    ...
    f.selectedCreateExpansion = GuildWorkshop_GetDefaultExpansion()
    f.selectedCreatePhase = GuildWorkshop_GetDefaultPhase()
    ...
    panel.expansionDropdown = CreateFrame("Frame", "GuildWorkshopExpansionDropdown", panel, "UIDropDownMenuTemplate")
    ...
    UIDropDownMenu_Initialize(panel.expansionDropdown, function()
        GuildWorkshop_ClearDropdownItemTooltips()
    ...
    panel.professionDropdown = CreateFrame("Frame", "GuildWorkshopProfessionDropdown", panel, "UIDropDownMenuTemplate")
    ...
    UIDropDownMenu_Initialize(panel.professionDropdown, function()
        GuildWorkshop_ClearDropdownItemTooltips()
        local info = UIDropDownMenu_CreateInfo()
        for _, profession in ipairs(GuildWorkshop_GetProfessionOptions()) do
            info = UIDropDownMenu_CreateInfo()
    ...
    panel.createBtn:SetScript("OnClick", function()
        local _, err = GuildWorkshop_CreateWorkshop(
        ...
        if err then
            ...
            print("|cff00ccffGuild Workshop|r Workshop created.")
            self:Refresh()
        end
    end)
    ...
    panel.joinDropdown = CreateFrame("Frame", "GuildWorkshopJoinDropdown", panel, "UIDropDownMenuTemplate")
    ...
    UIDropDownMenu_Initialize(panel.joinDropdown, function()
        ...
        for _, openRoom in ipairs(UI.joinRoomOptions or {}) do
            ...
            info.text = string.format(
                "%s  |cff888888(%s)|r",
                openRoom.name,
                GuildWorkshop_FormatWorkshopScope(openRoom)
            )
            MarkDropdownSelection(info, GuildWorkshop_GetActiveRoomId() == openRoom.id)
            info.func = function()
                local _, err = GuildWorkshop_JoinWorkshop(openRoom.id)
                if err then
                    print("|cff00ccffGuild Workshop|r " .. err)
                else
                    print("|cff00ccffGuild Workshop|r Selected " .. openRoom.name .. ".")
    ...
    panel.changeWorkshopBtn:SetScript("OnClick", function()
        SafeCloseDropdownMenus()
        local _, err = GuildWorkshop_DeselectWorkshop()
        if err then
            print("|cff00ccffGuild Workshop|r " .. err)
        else
            print("|cff00ccffGuild Workshop|r Workshop deselected. You remain a member — select another below.")
    ...
    local scroll = CreateFrame("ScrollFrame", "GuildWorkshopWorkshopMembersScroll", panel, "UIPanelScrollFrameTemplate")
...
function UI:RefreshWorkshopMembers()
    ...
    if room then
        local crafterPlural = GuildWorkshop_GetCrafterLabel(GuildWorkshop_GetRoomProfession(room))
    ...
    local isManager = GuildWorkshop_CanManageWorkshop(room, player)
    local isOwner = GuildWorkshop_IsWorkshopOwner(room, player)
    local members = GuildWorkshop_GetSortedRoomMembers(room)
    local crafterSingular = GuildWorkshop_GetRoomCrafterSingular(room)
    local crafterShort = GuildWorkshop_GetRoomCrafterShortLabel(room)
    local y = 0
    for _, name in ipairs(members) do
        ...
        label:SetText(GuildWorkshop_ColorizePlayer(name) .. tagText)
        ...
        if canManageJC or canManageCoLeader then
            ...
            if canManageCoLeader and not isTargetCoLeader then
                placeBtn("Make co-leader", 124, function()
                    local ok, err = GuildWorkshop_AddCoLeader(room.id, name)
                    if not ok then
                        print("|cff00ccffGuild Workshop|r " .. (err or "Action failed."))
                    else
                        self:Refresh()
                    end
                end)
            ...
            if canManageJC then
                if isJewelcrafter then
                    placeBtn("Demote " .. crafterShort, 88, function()
                        local ok, err = GuildWorkshop_DemoteCollaborator(room.id, name)
                        if not ok then
                            print("|cff00ccffGuild Workshop|r " .. (err or "Action failed."))
                        else
                    ...
                    placeBtn("Make " .. crafterShort, 88, function()
                        local ok, err = GuildWorkshop_PromoteMember(room.id, name)
                        if not ok then
                            print("|cff00ccffGuild Workshop|r " .. (err or "Action failed."))
                        else
                            self._requestWorkshopSync = false
            ...
            if canManageCoLeader and isTargetCoLeader then
                placeBtn("Demote co-leader", 124, function()
                    local ok, err = GuildWorkshop_RemoveCoLeader(room.id, name)
                    if not ok then
                        print("|cff00ccffGuild Workshop|r " .. (err or "Action failed."))
                    else
                        self:Refresh()
...
function UI:RefreshWorkshopPanel()
    ...
    if room then
        ...
        if room.leader == player then
            role = "Leader"
        elseif GuildWorkshop_IsRoomCoLeader(room, player) then
            role = "Co-leader"
        end
        ...
        panel.status:SetText(string.format(
            ...
            room.name,
            GuildWorkshop_FormatWorkshopScope(room),
            GuildWorkshop_ColorizePlayer(room.leader),
            role
        ))
    else
    ...
    elseif #GuildWorkshop_GetOpenRooms() == 0 then
    ...
    local canManageWorkshop = room and GuildWorkshop_CanManageWorkshop(room, UnitName("player"))
...
function UI:CreateStockPanel()
    ...
    panel.refreshBtn:SetScript("OnClick", function()
        local room = GuildWorkshop_GetActiveRoom()
        if room and not GuildWorkshop_IsWorkshopFullySupported(room) then
            print("|cff00ccffGuild Workshop|r " .. GuildWorkshop_GetUnsupportedWorkshopMessage(room))
            return
        end
        if GuildWorkshop_ShouldShareStock() then
            GuildWorkshop_RefreshLocalStock()
            print("|cff00ccffGuild Workshop|r Material stock scanned and shared with the workshop.")
        elseif GuildWorkshop.UI then
            GuildWorkshop_RefreshUI()
            print("|cff00ccffGuild Workshop|r Material stock refreshed.")
        end
    end)

    local scroll = CreateFrame("ScrollFrame", "GuildWorkshopStockScroll", panel, "UIPanelScrollFrameTemplate")
...
function UI:RefreshStockPanel()
    ...
    if not GuildWorkshop_IsWorkshopFullySupported(room) then
        panel.title:SetText("Material Stock")
        ...
        empty:SetText(GuildWorkshop_GetUnsupportedWorkshopMessage(room))
        table.insert(panel.stockLines, empty)
        panel.content:SetHeight(80)
        return
    end

    panel.title:SetText(GuildWorkshop_IsJewelcraftingWorkshop(room) and "Gem Stock Overview"
        or (GuildWorkshop_GetRoomScarceMaterialName(room) or "Material") .. " Stock")
    ...
    local function AddGemLines(counts, indent)
        indent = indent or 16
        local listing = GuildWorkshop_GetStockListing(counts, GuildWorkshop_GetRoomProfession(room))
        if #listing == 0 then
            local empty = parent:CreateFontString(nil, "ARTWORK", "GameFontDisable")
        ...
        for _, entry in ipairs(listing) do
            ...
            if entry.itemId then
                GuildWorkshop_AttachItemTooltip(row, function()
                    return entry.itemId
                end)
            end
    ...
    local function AddSection(title, counts, style, opts)
        ...
        elseif style == "jc" then
            ...
            local nameText = GuildWorkshop_ColorizePlayer(title)
    ...
    local player = UnitName("player")
    local canShare = GuildWorkshop_ShouldShareStock()
    if canShare then
        local personalTitle = "Your Bags + Bank"
        local bankNote = GuildWorkshop_GetBankStockNote and GuildWorkshop_GetBankStockNote()
        if bankNote then
            personalTitle = personalTitle .. " |cff888888(" .. bankNote .. ")|r"
        end
        AddSection(personalTitle, GuildWorkshop_GetStockCounts("personal"), "personal")
    end
    ...
    AddSection("Workshop Total", GuildWorkshop_GetAggregatedWorkshopStock(room), "total")

    panel.content:SetHeight(math.max(400, y + 20))
end

function UI:CreateRecipesPanel()
    if GuildWorkshop_InitRecipeEvents then
        GuildWorkshop_InitRecipeEvents()
    end
    local f = self.frame
    local panel = CreateInsetPanel(f)
    panel:SetPoint("TOPLEFT", 12, -108)
    ...
    panel.refreshBtn = CreateFrame("Button", "GuildWorkshopRecipesRefresh", panel, "SecureActionButtonTemplate, UIPanelButtonTemplate")
    panel.refreshBtn:SetSize(100, 22)
    panel.refreshBtn:SetPoint("TOPRIGHT", -16, -14)
    panel.refreshBtn:SetText("Refresh")
    panel.refreshBtn:SetScript("OnShow", function(self)
        if GuildWorkshop_InitRecipesRefreshButton then
            GuildWorkshop_InitRecipesRefreshButton(self)
        end
    end)

    local scroll = CreateFrame("ScrollFrame", "GuildWorkshopRecipesScroll", panel, "UIPanelScrollFrameTemplate")
...
function UI:RefreshRecipesPanel()
    ...
    if not GuildWorkshop_IsWorkshopFullySupported(room) then
        panel.refreshBtn:Disable()
        panel.help:SetText(GuildWorkshop_GetUnsupportedWorkshopMessage(room))
        local empty = parent:CreateFontString(nil, "ARTWORK", "GameFontHighlight")
        empty:SetPoint("TOPLEFT", 8, -8)
        empty:SetWidth(FRAME_WIDTH - 88)
        empty:SetJustifyH("LEFT")
    ...
    panel.refreshBtn:Enable()
    local isJewelcrafting = GuildWorkshop_IsJewelcraftingWorkshop(room)
    ...
    if isJewelcrafting then
        ...
        panel.help:SetText("Phase 3 Heart of Darkness crafts known by promoted "
            .. string.lower(GuildWorkshop_GetCrafterLabel(GuildWorkshop_GetRoomProfession(room))) .. ".")
    end
    ...
    local tier = panel.recipeSubTab or "epic"
    local y = 8
    for _, entry in ipairs(GuildWorkshop_GetRecipeCoverage(room, tier)) do
        if entry.craft then
            ...
            GuildWorkshop_AttachItemTooltip(nameBtn, function()
                return craft.itemId
            end)
            ...
            if hasRecipe then
                ...
                for _, crafterName in ipairs(entry.jcs) do
                    table.insert(names, GuildWorkshop_ColorizePlayer(crafterName))
                end
            ...
            if hasRecipe then
                ...
                for _, jcName in ipairs(entry.jcs) do
                    table.insert(names, GuildWorkshop_ColorizePlayer(jcName))
                end
...
function UI:PopulateGearDropdown(dropdown, frameRef)
    ...
    if not category then
        ...
        info.notCheckable = true
        UIDropDownMenu_AddButton(info)
        GuildWorkshop_ApplyDropdownItemTooltips(indexToItemId)
        return
    end

    if category == "PVE" then
        for _, raid in ipairs(GuildWorkshop_GearRaids) do
            ...
            for _, gear in ipairs(GuildWorkshop_GetGearByRaid(raid)) do
                local info = UIDropDownMenu_CreateInfo()
        ...
        for _, gear in ipairs(GuildWorkshop_GetGearByClass(className)) do
            local info = UIDropDownMenu_CreateInfo()
            info.text = gear.label
            info.value = gear.itemId
            MarkDropdownSelection(info, frameRef.selectedGear and frameRef.selectedGear.itemId == gear.itemId)
...
function UI:CreateGearDropdown(name, parent, point, x, y, frameRef, width)
    ...
    GuildWorkshop_AttachDropdownTooltips(dropdown, function()
...
function UI:CreateGearTypeDropdown(name, parent, point, x, y, frameRef, width)
    ...
    UIDropDownMenu_Initialize(dropdown, function()
        ...
        for _, category in ipairs(GuildWorkshop_GearCategories) do
            local info = UIDropDownMenu_CreateInfo()
            info.text = category
...
function UI:CreateGearClassDropdown(name, parent, point, x, y, frameRef, width)
    ...
    UIDropDownMenu_Initialize(dropdown, function()
        ...
        for _, className in ipairs(GuildWorkshop_PvpClasses) do
            local info = UIDropDownMenu_CreateInfo()
            info.text = className
...
function UI:CreateGemDropdown(name, parent, point, x, y, onSelect, getSelected, width)
    ...
    UIDropDownMenu_Initialize(dropdown, function()
        ...
        for _, entry in ipairs(self.gemMenuEntries) do
            ...
            elseif entry.kind == "gem" then
                local gem = entry.gem
                local label = GuildWorkshop_FormatGemDropdownLabel(gem.name) or gem.name
                info.text = label
    ...
    SetDropdownDisplayText(dropdown, GEM_PLACEHOLDER)
    GuildWorkshop_AttachDropdownTooltips(dropdown, function()
        return GuildWorkshop_GetGemItemId(getSelected())
    end)
    ConfigureOrderDropdown(dropdown, width)
    return dropdown
end
...
function UI:RefreshGemDropdownLabels()
    ...
    for i = 1, 3 do
        ...
        if dropdown and gemName and gemName ~= "None" then
            SetDropdownDisplayText(dropdown, GuildWorkshop_FormatGemDropdownLabel(gemName) or gemName)
        end
    end
end
...
function UI:ConfigureOrderFormMode(room)
    ...
    local isCraft = room and GuildWorkshop_IsHoDProfession(GuildWorkshop_GetRoomProfession(room))
    ...
    if isCraft then
        ...
        if not f.selectedCraftCategory then
            local categories = GuildWorkshop_GetCraftCategories(GuildWorkshop_GetRoomProfession(room))
...
function UI:PopulateCraftDropdown(dropdown, frameRef)
    ...
    local professionId = GuildWorkshop_GetRoomProfession(room)
    GuildWorkshop_ClearDropdownItemTooltips()
    local buttonIndex = 0
    local indexToItemId = {}
    ...
    for _, craft in ipairs(GuildWorkshop_GetCraftsByCategory(professionId, category)) do
        local info = UIDropDownMenu_CreateInfo()
        info.text = GuildWorkshop_FormatCraftLabel(craft)
        ...
        info.func = function()
            ...
            if frameRef.craftWarning then
                if GuildWorkshop_WorkshopHasRecipeForCraft(craft.name) then
                    frameRef.craftWarning:Hide()
                else
...
function UI:CreateOrderDialog()
    ...
    local dialog = CreateFrame("Frame", "GuildWorkshopOrderDialog", f)
    ...
    dialog:Hide()
    GuildWorkshop_RegisterEscapeFrame(dialog)
...
function UI:OpenOrderDialog()
    if not GuildWorkshop_HasJoinedWorkshop() then
        print("|cff00ccffGuild Workshop|r Select a workshop before placing an order.")
        return
    end
    local room = GuildWorkshop_GetActiveRoom()
    if room and not GuildWorkshop_IsWorkshopFullySupported(room) then
...
function UI:RefreshOrderFormWarnings()
    ...
    if f.orderFormMode == "craft" then
        if f.selectedCraft and f.craftWarning then
            if GuildWorkshop_WorkshopHasRecipeForCraft(f.selectedCraft.name) then
                f.craftWarning:Hide()
            else
...
function UI:CloseOrderDialog()
    if GuildWorkshop_IsLoggingOut and GuildWorkshop_IsLoggingOut() then
        if self.frame and self.frame.orderDialog then
...
function UI:CreateOrderForm()
    ...
    f.gearClassDropdown = self:CreateGearClassDropdown(
        "GuildWorkshopGearClassDropdown",
        inset,
        "TOPLEFT",
        90,
        y + 8,
    ...
    f.craftCategoryDropdown = CreateFrame("Frame", "GuildWorkshopCraftCategoryDropdown", inset, "UIDropDownMenuTemplate")
    ...
    UIDropDownMenu_Initialize(f.craftCategoryDropdown, function()
        ...
        for _, category in ipairs(GuildWorkshop_GetCraftCategories(GuildWorkshop_GetRoomProfession(room))) do
            info = UIDropDownMenu_CreateInfo()
            info.text = category
    ...
    f.craftLabel:Hide()
    f.craftDropdown = CreateFrame("Frame", "GuildWorkshopCraftDropdown", inset, "UIDropDownMenuTemplate")
    ...
    GuildWorkshop_AttachDropdownTooltips(f.craftDropdown, function()
    ...
    f.selectedRole = nil
    f.roleDropdown = CreateFrame("Frame", "GuildWorkshopRoleDropdown", inset, "UIDropDownMenuTemplate")
    ...
    UIDropDownMenu_Initialize(f.roleDropdown, function()
        ...
        for _, role in ipairs(GuildWorkshop_ROLES) do
            info = UIDropDownMenu_CreateInfo()
            info.text = role
    ...
    f.submitBtn:SetScript("OnClick", function()
        local _, err
        if f.orderFormMode == "craft" then
            _, err = GuildWorkshop_CreateCraftOrder(
                f.selectedCraft,
                GetNotesInputText(f.notesInput),
                f.selectedRole
            )
        else
            _, err = GuildWorkshop_CreateOrder(
                f.selectedGear,
                f.selectedGems,
                GetNotesInputText(f.notesInput),
                f.selectedRole
        ...
        if err then
            ...
            print("|cff00ccffGuild Workshop|r Order submitted!")
            f.selectedGearCategory = nil
            f.selectedGearClass = nil
            f.selectedGear = nil
            f.selectedCraftCategory = nil
...
function UI:UpdateGemRecipeWarning(index)
    ...
    if gemName == "None" or GuildWorkshop_WorkshopHasRecipeForGem(gemName) then
        warningFrame:Hide()
        return
    end
...
function UI:CreateQueueList()
    ...
    local scroll = CreateFrame("ScrollFrame", "GuildWorkshopScroll", inset, "UIPanelScrollFrameTemplate")
...
function UI:CreateGemIconRow(parent, order, yOffset, startX)
    ...
    for i, gemName in ipairs(order.gems or {}) do
        ...
        local itemId = GuildWorkshop_GetGemItemId(gemName)
        if itemId then
            ...
            GuildWorkshop_AttachItemTooltip(btn, function()
                return itemId
            end)
            ...
            GuildWorkshop_AttachItemTooltip(labelBtn, function()
                return itemId
            end)
...
function UI:IsCraftOrder(order)
    ...
    return GuildWorkshop_GetCraftByItemId(order.itemId) ~= nil
        or GuildWorkshop_GetCraftByName(order.item) ~= nil
end
...
function UI:CreateOrderRow(order, yOffset)
    ...
    title:SetText(string.format(
        "%s  %s  [%s]",
        GuildWorkshop_ColorizePlayer(order.player, order.class),
        GuildWorkshop_GetRoleLabel(order.role),
        GuildWorkshop_GetOrderStatusLabel(order)
    ))
    title:SetWidth(contentWidth - 170)
    title:SetJustifyH("LEFT")

    local canManage = GuildWorkshop_CanManageOrder(order)
    local actionBtn
    ...
    if canManage and order.status ~= "completed" and order.status ~= "cancelled" then
        ...
        if order.status == "pending" then
            ...
            actionBtn:SetScript("OnClick", function()
                GuildWorkshop_UpdateStatus(order.id, "in_progress")
                self:Refresh()
            end)
        elseif order.status == "in_progress" then
            actionBtn = CreateFrame("Button", nil, btnArea, "UIPanelButtonTemplate")
            ...
            actionBtn:SetScript("OnClick", function()
                GuildWorkshop_UpdateStatus(order.id, "completed")
                self:Refresh()
            end)
        end
        ...
        if actionBtn and order.status ~= "completed" then
            ...
            downBtn:SetScript("OnClick", function()
                local ok, err = GuildWorkshop_MoveOrder(order.id, 1)
    ...
    local gearItemId = order.itemId or GuildWorkshop_GetGearItemId(order.item)
    ...
    if gearItemId then
        ...
        GuildWorkshop_AttachItemTooltip(gearIcon, function()
    ...
    if gearItemId or order.itemLink then
        GuildWorkshop_AttachItemTooltip(itemBtn, function()
            if order.itemLink then
                return order.itemLink
            end
            return gearItemId
    ...
    local canManageWorkshop = room and GuildWorkshop_CanManageWorkshop(room, player)
    ...
    if canManageWorkshop then
        ...
        deleteBtn:SetScript("OnClick", function()
            local ok, err = GuildWorkshop_DeleteOrder(order.id)
            if not ok and err then
                print("|cff00ccffGuild Workshop|r " .. err)
            end
            self:Refresh()
    ...
    elseif (isOwner or canManage) and order.status == "pending" then
        ...
        cancelBtn:SetScript("OnClick", function()
            GuildWorkshop_CancelOrder(order.id)
            self:Refresh()
        end)
    end
...
function UI:Refresh()
    if not self.frame or (GuildWorkshop_IsLoggingOut and GuildWorkshop_IsLoggingOut()) then
        return
    end
    ...
    local room = GuildWorkshop_GetActiveRoom()
    if room and GuildWorkshop_IsPromotedJewelcrafter(room, UnitName("player")) then
        local crafterSingular = GuildWorkshop_GetRoomCrafterSingular(room)
    ...
    if GuildWorkshop_HasJoinedWorkshop() and room then
    ...
    elseif self.activeTab == "stock" and GuildWorkshop_HasJoinedWorkshop() then
        self:RefreshStockPanel()
    elseif self.activeTab == "recipes" and GuildWorkshop_HasJoinedWorkshop() then
        self:RefreshRecipesPanel()
    end
    ...
    if self.activeTab == "orders" and GuildWorkshop_HasJoinedWorkshop() then
        local orders = GuildWorkshop_GetActiveOrders()
        local y = 0
        local lastGroup = nil
    ...
    elseif self.activeTab == "completed" and GuildWorkshop_HasJoinedWorkshop() then
        local orders = GuildWorkshop_GetCompletedOrders()
        local y = 0
        for _, order in ipairs(orders) do
            self:CreateOrderRow(order, y)
            y = y + self:GetOrderRowHeight(order) + ORDER_ROW_GAP
