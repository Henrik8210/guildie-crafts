...
GuildWorkshop_CutGemsCache = nil
GuildWorkshop_RareCutGemsCache = nil

GuildWorkshop_GemByName = {}
GuildWorkshop_GemByItemId = {}
for _, gem in ipairs(GuildWorkshop_Gems) do
    GuildWorkshop_GemByName[gem.name] = gem
    if gem.itemId then
        GuildWorkshop_GemByItemId[gem.itemId] = gem
    end
end

function GuildWorkshop_GetCutGems()
    if GuildWorkshop_CutGemsCache then
        return GuildWorkshop_CutGemsCache
    end
    local list = {}
    for _, gem in ipairs(GuildWorkshop_Gems) do
    ...
    GuildWorkshop_CutGemsCache = list
    return list
end

function GuildWorkshop_GetEpicCutGems()
    return GuildWorkshop_GetCutGems()
end

function GuildWorkshop_GetRareCutGems()
    if GuildWorkshop_RareCutGemsCache then
        return GuildWorkshop_RareCutGemsCache
    end
    local list = {}
    for _, gem in ipairs(GuildWorkshop_Gems) do
    ...
    GuildWorkshop_RareCutGemsCache = list
    return list
end

function GuildWorkshop_GetRawGems()
    local list = {}
    for _, gem in ipairs(GuildWorkshop_Gems) do
        if gem.raw then
            table.insert(list, gem)
...
function GuildWorkshop_GetGemColor(name)
    local gem = GuildWorkshop_GemByName[name]
    return gem and gem.color or "Unknown"
end

function GuildWorkshop_GetRecipeGems(tier)
    if tier == "rare" then
        return GuildWorkshop_GetRareCutGems()
    end
    return GuildWorkshop_GetEpicCutGems()
end

function GuildWorkshop_IsTrackedRecipeItemId(itemId)
    local gem = GuildWorkshop_GemByItemId[itemId]
    if gem and not gem.raw then
        return true
    end
    if GuildWorkshop_IsTrackedHoDCraftItemId then
        return GuildWorkshop_IsTrackedHoDCraftItemId(itemId)
    end
    return false
end

function GuildWorkshop_FormatGemLabel(gem)
    return gem.name
end
