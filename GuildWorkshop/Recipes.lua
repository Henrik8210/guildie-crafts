...
local function EnsureDB()
    ...
    GuildWorkshopDB.recipes.jcReports = GuildWorkshopDB.recipes.jcReports or {}
end
...
local function GetProfessionLabel(professionId)
    return GuildWorkshop_GetProfessionLabel(professionId or "jewelcrafting")
end

function GuildWorkshop_ShouldShareRecipes()
    return GuildWorkshop_ShouldShareStock()
end

function GuildWorkshop_IsJewelcraftingOpen()
    return GuildWorkshop_IsTradeSkillOpen("jewelcrafting")
end

function GuildWorkshop_IsTradeSkillOpen(professionId)
...
function GuildWorkshop_HandleRecipesRefreshClick()
    local professionId = GetActiveRecipeProfession()
    if GuildWorkshop_IsTradeSkillOpen(professionId) then
        GuildWorkshop_RefreshRecipesFromMacro(false)
        return
    end
    ...
    GuildWorkshop_RefreshRecipesFromMacro(true)
end

function GuildWorkshop_InitRecipesRefreshButton(btn)
    ...
    btn:SetAttribute("macrotext", "/run GuildWorkshop_HandleRecipesRefreshClick()")
end

function GuildWorkshop_RefreshRecipesSyncOnly()
    if GuildWorkshop.Sync then
        GuildWorkshop.Sync:RequestSync()
    end
    if GuildWorkshop.UI then
        GuildWorkshop_RefreshUI()
    end
    print("|cff00ccffGuild Workshop|r Recipe list refreshed.")
end
...
local function MatchGemByName(name)
    ...
    local gem = GuildWorkshop_GemByName[name]
    if gem then
        return gem
    end
    if strlower then
        for gemName, entry in pairs(GuildWorkshop_GemByName or {}) do
...
local function MatchCraftByName(name)
    ...
    local craft = GuildWorkshop_GetCraftByName(name)
    if craft then
        return craft
    end
    if strlower then
        for craftName, entry in pairs(GuildWorkshop_CraftByName or {}) do
...
function GuildWorkshop_ScanOpenTradeSkill()
    local known = {}
    ...
    for i = 1, num do
        ...
        if name and skillType ~= "header" then
            ...
            if itemId and GuildWorkshop_IsTrackedRecipeItemId(itemId) then
                known[itemId] = true
            end
        end
    end
...
local function StoreLocalRecipeReport(itemIds)
    EnsureDB()
    GuildWorkshopDB.recipes.jcReports[UnitName("player")] = {
        itemIds = itemIds,
        updatedAt = time(),
    }
end
...
local function ApplyRecipeScanResult(known, numSkills, source)
    ...
    GuildWorkshop.Sync:BroadcastRecipes(known)
    if GuildWorkshop.UI then
        GuildWorkshop_RefreshUI()
    end
    ...
    if scanPending then
        if numSkills <= 0 then
            print("|cffff0000Guild Workshop|r Could not read recipes. Open your "
                .. professionLabel .. " window and try again.")
        elseif knownCount == 0 then
            print("|cff00ccffGuild Workshop|r Scanned " .. professionLabel
                .. " (" .. numSkills .. " entries). No tracked recipes found yet.")
        else
            print("|cff00ccffGuild Workshop|r Scanned and shared " .. knownCount
                .. " recipes with the workshop.")
        end
    elseif source == "login" and knownCount > 0 then
        print("|cff00ccffGuild Workshop|r Shared " .. knownCount .. " recipes with the workshop.")
    end
...
local function RunRecipeScan(source)
    if not GuildWorkshop_ShouldShareRecipes() then
        scanPending = false
        FinishAutoScanClose()
        return
    end

    local known, numSkills = GuildWorkshop_ScanOpenTradeSkill()
    if numSkills > 0 then
...
function GuildWorkshop_ScanAndShareRecipes()
    EnsureDB()
    if not GuildWorkshop_ShouldShareRecipes() then
        local crafters = string.lower(GuildWorkshop_GetCrafterLabel(GetActiveRecipeProfession()))
    ...
    print("|cff00ccffGuild Workshop|r Open "
        .. GetProfessionLabel(GetActiveRecipeProfession())
        .. ", then click Refresh on the Recipes tab.")
    return false, "closed"
end

function GuildWorkshop_ShareWorkshopRecipes()
    EnsureDB()
    if not GuildWorkshop_ShouldShareRecipes() then
        return
    end
    local player = UnitName("player")
    local report = GuildWorkshopDB.recipes.jcReports[player]
    if report and report.itemIds then
        GuildWorkshop.Sync:BroadcastRecipes(report.itemIds)
    end
end

function GuildWorkshop_AutoScanRecipes(silent)
    if not GuildWorkshop_ShouldShareRecipes() then
        return
    end

    local professionId = GetActiveRecipeProfession()
    if not GuildWorkshop_IsTradeSkillOpen(professionId) then
        if not silent then
            print("|cff00ccffGuild Workshop|r Open "
...
function GuildWorkshop_RequestRecipeScan()
    return GuildWorkshop_ScanAndShareRecipes()
end

function GuildWorkshop_ApplyRecipeReport(player, itemIds)
    EnsureDB()
    GuildWorkshopDB.recipes.jcReports[player] = {
        itemIds = itemIds,
        updatedAt = time(),
    }
    if GuildWorkshop.UI then
...
function GuildWorkshop_GetRecipeCoverage(room, tier)
    EnsureDB()
    local coverage = {}
    if not room then
        return coverage
    ...
    local profession = GuildWorkshop_GetRoomProfession(room)
    if profession ~= "jewelcrafting" then
        for _, craft in ipairs(GuildWorkshop_GetCraftCatalog(profession)) do
            local jcs = {}
            for _, crafterName in ipairs(GuildWorkshop_GetWorkshopStockContributors(room)) do
    ...
    for _, gem in ipairs(GuildWorkshop_GetRecipeGems(tier or "epic")) do
        local jcs = {}
        for _, jcName in ipairs(GuildWorkshop_GetWorkshopStockContributors(room)) do
            local report = GuildWorkshopDB.recipes.jcReports[jcName]
...
function GuildWorkshop_WorkshopHasRecipeForGem(gemName)
    if not gemName or gemName == "None" then
        return true
    end

    local gem = GuildWorkshop_GemByName[gemName]
...
function GuildWorkshop_GetRecipeIndicatorText(gemName)
    if not gemName or gemName == "None" then
        return ""
    end

    local gem = GuildWorkshop_GemByName and GuildWorkshop_GemByName[gemName]
    if not gem or gem.raw or not gem.itemId then
        return ""
    end

    if GuildWorkshop_WorkshopHasRecipeForGem(gemName) then
...
function GuildWorkshop_FormatGemDropdownLabel(gemName)
    if not gemName or gemName == "None" then
        return nil
    end

    local indicator = GuildWorkshop_GetRecipeIndicatorText(gemName)
    if indicator == "" then
        return gemName
    end
...
local function QueuePassiveRecipeScan()
    if not GuildWorkshop_ShouldShareRecipes() or not GuildWorkshop_HasJoinedWorkshop() then
        return
    end
    ...
    local function run()
        ...
        if not GuildWorkshop_ShouldShareRecipes() or not GuildWorkshop_IsTradeSkillOpen(GetActiveRecipeProfession()) then
            return
        end
        local known, numSkills = GuildWorkshop_ScanOpenTradeSkill()
        if numSkills <= 0 then
...
function GuildWorkshop_RefreshRecipesFromMacro(delayScan)
    if not GuildWorkshop_ShouldShareRecipes() then
        GuildWorkshop_RefreshRecipesSyncOnly()
        return
    end

    if delayScan then
        scanPending = true
        print("|cff00ccffGuild Workshop|r Opening "
            .. GetProfessionLabel(GetActiveRecipeProfession()) .. "…")
    ...
    if numSkills > 0 then
        ...
        print("|cffff0000Guild Workshop|r Open your "
            .. GetProfessionLabel(GetActiveRecipeProfession())
...
function GuildWorkshop_InitRecipeEvents()
