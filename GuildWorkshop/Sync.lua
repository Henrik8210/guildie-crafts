...
local Sync = {}
GuildWorkshop.Sync = Sync

local PREFIX = "GuildWrk"
local MSG_ADD = "A"
local MSG_STATUS = "S"
local MSG_REMOVE = "R"
local MSG_REQUEST = "Q"
...
function Sync:BroadcastAllOrderQueues(requester)
    for _, room in pairs(GuildWorkshopDB.rooms or {}) do
        if room.id and room.orderQueue and GuildWorkshop_IsRoomMember(room, requester) then
...
function Sync:BroadcastRoom(room)
    ...
    local msg = table.concat({
        ...
        EncodeField(room.expansion or GuildWorkshop_GetDefaultExpansion()),
        EncodeField(room.phase or GuildWorkshop_GetDefaultPhase()),
...
function Sync:OnAddonMessage(prefix, message, channel, sender)
    ...
    if msgType == MSG_ADD then
        ...
        local room = roomId and GuildWorkshop_GetRoom(roomId)
        if not room or not GuildWorkshop_IsSameGuild(room) then
            return
        end
        if not GuildWorkshop_IsRoomMember(room, UnitName("player")) then
            return
        end
        local order = {
            id = parts[2],
        ...
        local existing = GuildWorkshopDB.orders[order.id]
        if existing then
    ...
    elseif msgType == MSG_REQUEST then
        ...
        for _, order in pairs(GuildWorkshopDB.orders or {}) do
            if order.roomId then
                ...
                if room and GuildWorkshop_IsSameGuild(room) and GuildWorkshop_IsRoomMember(room, requester) then
                    self:BroadcastOrder(order)
                end
            end
        end
        ...
        if GuildWorkshop_ShouldShareRecipes() then
            GuildWorkshop_ShareWorkshopRecipes()
        end
        self:BroadcastAllOrderQueues(requester)
    elseif msgType == MSG_ROOM then
        local guild = DecodeField(parts[8])
        if guild and guild ~= GuildWorkshop_GetGuildName() then
            return
        end
        local existingRoom = GuildWorkshopDB.rooms[parts[2]]
        ...
        local room = {
            ...
            guild = guild or GuildWorkshop_GetGuildName(),
            expansion = DecodeField(parts[11]) or GuildWorkshop_GetDefaultExpansion(),
            phase = DecodeField(parts[12]) or GuildWorkshop_GetDefaultPhase(),
            profession = DecodeField(parts[13]) or "jewelcrafting",
        }
        room.members[room.leader] = true
        GuildWorkshop_ApplyRoom(room)
    elseif msgType == MSG_JOIN then
        local room = GuildWorkshopDB.rooms[parts[2]]
        if not room then
        ...
        GuildWorkshop_ApplyRoom(room)
        if GuildWorkshop_IsRoomMember(room, UnitName("player")) then
            self:BroadcastRoom(room)
            if GuildWorkshop_CanManageWorkshop(room, UnitName("player")) then
    ...
    elseif msgType == MSG_COLLAB then
        ...
        if room then
            ...
            if parts[3] == "add" then
                ...
                if parts[4] == UnitName("player") and GuildWorkshop_ShouldShareStock() then
    ...
    elseif msgType == MSG_QUEUE then
        local room = GuildWorkshopDB.rooms[parts[2]]
        if room and GuildWorkshop_IsRoomMember(room, UnitName("player")) then
    ...
    elseif msgType == MSG_STOCK then
        ...
        if not room or not GuildWorkshop_HasJoinedWorkshop() then
            return
        end
        if not GuildWorkshop_AcceptsWorkshopStockReport(room, parts[2]) then
            return
        end
        GuildWorkshop_ApplyStockReport(parts[2], DecodeStock(parts[4]), parts[3])
    elseif msgType == MSG_RECIPES then
        ...
        GuildWorkshop_ApplyRecipeReport(parts[2], DecodeRecipeIds(parts[3]))
    end
end
